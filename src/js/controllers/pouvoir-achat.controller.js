...
import { analyserPouvoirAchat, getTopCommunes } from "../models/pouvoir-achat.model.js";
import { renderPouvoirAchatResults, hidePouvoirAchatResults } from "../views/pouvoir-achat.view.js";
...
export function initPouvoirAchatModal() {
  ...
  const closeResultsBtn = document.getElementById('close-pouvoir-achat-results');
...
async function analyserBudget(budget) {
  console.log(`Analyse du pouvoir d'achat avec un budget de ${budget}€`);
  ...
  if (currentScale === 'section' && state.currentCommune) {
    ...
    console.log(`Analyse au niveau SECTION (commune: ${state.currentCommuneName})`);
    ...
    console.log(`Analyse au niveau COMMUNE (département: ${state.currentDeptName})`);
    ...
    analysisType = 'global';
    console.log(`Analyse au niveau GLOBAL (Île-de-France)`);
  ...
  console.log(`Analyse terminée : ${results.length} zones analysées, top 5 affiché`);
}
...
function resetPouvoirAchatAnalysis() {
  ...
  console.log('Analyse de pouvoir d\'achat réinitialisée');
}
